...
impl RtdsEngine {
    ...
    pub async fn run(&self) {
        info!("Starting Data API poller (dynamic wallet list from WalletTracker)");

        let poll_interval = Duration::from_secs(2);
        let mut first_run = true;
        
        loop {
            // Read wallets dynamically from WalletTracker on every poll
            ...
            if watched_wallets.is_empty() {
                if first_run {
                    warn!("No wallets configured for tracking. Data API poller waiting...");
                    first_run = false;
                }
            ...
            match client.get(&url).send().await {
                ...
                Err(e) => {
                    ...
                    if attempts >= max_retries {
                        warn!("Failed to fetch activity for {} after {} retries: {}", wallet, max_retries, e);
                        break;
                    }
